$(function() {
	(function() {
		...
		$.topNav = function(menuCode, $html, frames) {
            if(typeof eventsManage != 'undefined'){
                try {
                eventsManage.trigger('positions',menuCode.html(),menuCode.attr('menu-code'),$html);
		if (/msie/.test(navigator.userAgent.toLowerCase())) {
			for(var i= 0,len=$('#content>.breadLine').find("span").length;i<len;i++){
				var maxWidth=$($('#content>.breadLine').find("span")[i]).css('max-width');
			    if((maxWidth!==undefined||maxWidth!==null)&&$($('#content>.breadLine').find("span")[i]).width()>=parseInt(maxWidth)){
				$($('#content>.breadLine').find("span")[i]).css('width',maxWidth);
			    }
			}		
		} 
...
var loader = (function($,loader) {
	return {
		...
		hide : function(callback) {
			if($.isOcx && $('#loaderTop').data('hasModal')){
                    $('#loaderTop').removeData('hasModal');
...
var progress = (function($,progress) {
	return {
		show : function(percent, elapsed,cancelOverlay, callback) {
			var elapsed = elapsed || language.text('loading');
			var percent = percent || 0;
            var cancelOverlay= cancelOverlay &&true;
            var temp=cancelOverlay==true? ' ':'<div class="shadow"></div>'
			var html = $('<div id="progressTop" class="progress-overlay"><div class="progress-m"><div class="progressbar"><div class="progress progress-small progress-info"><div class="filler"><div class="bar" style="width:'
					...
					+ elapsed
					+ '</div><div class="actions clear" style="display:none;"><a id="btn_progress_hide" href="javascript:void(0);" class="buttonS bBlue">'+ language.text('ui.progress.runBack')+'</a></div></div></div>'
                    +temp
                    +'</div>');
		...
		hide : function(callback) {
			$('#btn_progress_hide').attr('disabled','disabled');
