		...
		plot.hooks.bindEvents.push(function (plot, eventHolder) {
			...
			$(placeholder).bind("plothover", function (event, pos, item) {
				if (item) {					
                    ...
                    var w = $(window).width();
                    var h = $(window).height();
                    var eleW = $tip.width() + parseInt($tip.css('paddingLeft')) + parseInt($tip.css('paddingRight'));
                    var eleH = $tip.height() + parseInt($tip.css('paddingTop')) + parseInt($tip.css('paddingBottom'));
                    var tipPos = {};
                    tipPos = tipPosition;
                    if(((tipPos.x + to.shifts.x + eleW) > w) && tipPos.y + to.shifts.y > 0) {
                        tipPos.x = w - eleW;
                    }
                    if((tipPos.y + to.shifts.y + eleH) > h) {
                        tipPos.y = h - eleH - to.shifts.y;
                    }
                    if(tipPos.x + to.shifts.x < 0) {
                        	tipPos.x = 0 - to.shifts.x;
                    }
                    if(tipPos.y + to.shifts.y < 0) {
                        tipPos.y = 0 - to.shifts.y;
                    }
                    if(tipPos.y + to.shifts.y == 0) {
                        tipPos.x = tipPosition.x - to.shifts.x - eleW - 20;
                    }
                    tipPosition = tipPos;
                    if(tipPosition.x + to.shifts.x < 0){
                    	$tip.css({left: 0, top: tipPosition.y + to.shifts.y}).show();
                    }else
                    	$tip.css({left: tipPosition.x + to.shifts.x, top: tipPosition.y + to.shifts.y}).show();
				}
				else {
					$tip.hide().html('');
				}
