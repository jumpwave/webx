...
var LinkageSel = function(opts) {
	...
	this.st = {
			...
			defVal		: [],			// 默认选择项可多级
			data		: null,
			head		: false,		// {str|''|false} 自动添加第一个选择项,非字符串或false表示不使用
			level		: 20,			// 限定层级防止死循环
			loaderImg	: '/baseui/themes/default/images/elements/loaders/1s.gif',
	...
	if (selLen < 1) {
        if(window.console)
		console.info('no object has attached to mLinkageSel()!');
		return false;
	}
...
LinkageSel.prototype.fill = function (bindIdx, selValue) {
	...
	if (bindIdx >= st.level) {
		...
		this.custCallback();
        if(!that.st.initComplete) {
            setTimeout(function(){
                that.lastCallback.apply(that);
            },0);
            that.st.initComplete = true;
        }
		return false;
	}
		...
		for (var x in data) {
			...
			if (recycleLen > 0) {
				...
				if (typeof tOption === 'object') { 
					...
					tOption = jQuery(tOption).val(x).text(row[name]).prop('selected', false) ;	// for jQuery 1.6+
				}
				else {
					tOption = jQuery('<option>').val(x).text(row[name]);
				}
				recycleLen--;
			}
			else {
			...
			tOption = tOption.attr('title', row[name]).get(0);
			tarr.push(tOption);
...
// 执行用户自定义change事件末回调函数
LinkageSel.prototype.lastCallback = function() {
    var st = this.st;
    if (!st.trigger) { return;}
    ...
    if (st.callback && typeof st.callback === 'function') {
        st.callback.apply(this.outer);		// outer回调函数中this即为实例对象
    }
};
