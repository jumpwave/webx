define('./ocx_playback', [ './active_x' ], function(require, exports) {
	...
	return Active_x.extend({
		...
		handleException : function(e) {
			...
			el.find('*').hide();
			if(isExist)
				el.append('<div class="msg-b msg-b-stop"><i></i><div class="msg-cnt"><span>' + language.text('ocx.error')+'</span></div></div>');
			else{
				el.append('<div class="msg-b msg-b-stop"><i></i><div class="msg-cnt"><span>'+language.text('ocx.error')+'</span><p>'+language.text('ocx.nonExist') + '</p></div></div>');
			}
			if(isExist && (this.msgEl && this.msgEl.length>0)&&(typeof currentIsDialog == 'undefined'||currentIsDialog==false)){
				...
				setTimeout(function(){//转15秒钟后，自动隐藏
					loader.hide();
					jAlert(language.text('ocx.load.error'));
				}, 15000);
			}
			return false;
		}
