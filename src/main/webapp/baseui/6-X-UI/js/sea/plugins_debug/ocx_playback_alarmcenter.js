define('./ocx_playback_alarmcenter', ['./ocx_playback'], function(require, exports) {
	...
	return Ocx_playback.extend({
		...
		startPlay : function(cameraId, startTime, stopTime, name) {
			...
			_self.setWindowStatus('', language.text('ocx.searchRecord'), true);
			$.ajax({
					...
					if (result.success && result.ret == 1) {
						if (result.recordItems && result.recordItems.length > 0) {
							...
							if (record.status != '200') {
								_self.setWindowStatus('', language.text('ocx.searchRecordF'));
							} else if (record.total == 0) {
								_self.setWindowStatus('', language.text('ocx.noRecord'));
							} else {
								_self.setWindowStatus('', language.text('ocx.playRecord'), true);
								_self.setResult(0, result.recordItems);
								_self.startPlayback(0);
							}
						} else {
							_self.setWindowStatus(0, language.text('ocx.searchRecordF'));
						}
					} else {
						$.sticky(language.text('ocx.searchRecordF'), {
							type : "error"
						});
					}
				}
