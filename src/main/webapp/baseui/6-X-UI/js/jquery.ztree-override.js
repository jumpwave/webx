...
(function($) {
    ...
    var tools = $.fn.zTree._z.tools,handler = $.fn.zTree._z.handler,$$ = tools.$;
    ...
    data.addZTreeTools(function(setting, zTreeTools) {
        ...
        zTreeTools.setOneGenerationSonNode = function(node,value,srcNode){
              return view.setOneGenerationSonNode(this.setting, node, value, srcNode);
        }

        zTreeTools.setAllSonNode = function(node,value,srcNode){
            return view.setAllSonNode(this.setting, node, value, srcNode);
        }
    });
   ...
   view.makeDOMNodeNameBefore = function(html, setting, node) {
      ...
      if (node.extra) {
        ...
        if (tools.apply(setting.view.showTitle, [ setting.treeId, node ], setting.view.showTitle) && title) {
            html.push("title='", title.replace(/'/g, "&#39;").replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/&/g,'&amp;'), "'");
        }
        html.push(">");
    };
    ...
    view.setAllSonNode = function(setting, node ,value, srcNode){
         view.checkNodeRelation(setting, node);
         view.setSonNodeCheckBox(setting, node ,value, srcNode);
        view.repairParentChkClassWithSelf(setting, node);

    }
    view.setOneGenerationSonNode  = function(setting, node ,value, srcNode){
        if (!node){
            return;
        }
        ...
        if (node[childKey]) {
            ...
            for ( var i = 0, l = node[childKey].length; i < l && chkDisabledCheckedInherit; i++) {
                var sNode = node[childKey][i],checkObj = $("#" + sNode.tId + consts.id.CHECK);
                    view.checkNodeRelation(setting, sNode);
                    if (sNode != data.getRoot(setting) && sNode.chkDisabled !== true) {
                        if (hasDisable && node.nocheck !== true) {
                            data.makeChkFlag(setting, sNode);
                        }
                        if (sNode.nocheck !== true) {
                            sNode[checkedKey] = value;
                            if (!hasDisable) sNode.check_Child_State = (sNode[childKey] && sNode[childKey].length > 0) ? (value ? 2 : 0) : -1;
                        } else {
                            sNode.check_Child_State = -1;
                        }
                        view.setChkClass(setting, checkObj, sNode);
                        if (setting.check.autoCheckTrigger && node != srcNode && node.nocheck !== true) {
                            setting.treeObj.trigger(consts.event.CHECK, [setting.treeId, sNode]);
                        }
                    }
                    view.repairParentChkClassWithSelf(setting, sNode);
    ...
    $.fn.tree = function(setting, zNodes) {
        ...
        if (setting.ajax) {
            ...
            $.extend(setting.ajax, {
                ...
                error : function() {
                    errorHandle(language.text('tree.error'));
                    if (error) {
