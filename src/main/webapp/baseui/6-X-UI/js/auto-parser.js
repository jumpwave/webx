$(function() {
    ...
    $(document).on('submit doSubmit','form[history=true]',function(){
        var self = $(this),action = $.absURL(self.attr('action') || location.href),index = action.lastIndexOf('/'),lIndex = action.lastIndexOf('?'),isIE6 = ($.browser.msie && ($.browser.version == "6.0"));
        if(isIE6 && memory) {
            memory.removeItem(action.substring(index + 1, lIndex > -1 ? lIndex : action.length + 1));
            memory.setItem(action.substring(index + 1,  lIndex > -1 ? lIndex : action.length + 1),JSON.stringify(self.formToArray()));
        }else {
            memory.removeItem(action);
    ...
    if(typeof pt != "undefined") {
        if (pt.history) {
            ...
            $('form[history=true]').$each(function(i, $form) {
                var action = $.absURL($form.attr('action') || location.href),index = action.lastIndexOf('/'),lIndex = action.lastIndexOf('?');
	...
	function paramToForm() {
		...
		$('form[history=true]').$each(function(i, $form) {
            ...
            var datas = null;
            if($.browser.msie && ($.browser.version == "6.0"))
                datas = JSON.parse(memory.getItem(action.substring(index + 1, lIndex > -1 ? lIndex : action.length + 1)));/*historyMap[action];*/
            else
                datas =  JSON.parse(memory.getItem(action));
            if (datas == null) {
                return;
            }
	...
	$.fn.submitHistory = function() {
        var $form = this, action = $.absURL($form.attr('action') || location.href),index = action.lastIndexOf('/'),lIndex = action.lastIndexOf('?'),isIE6 = ($.browser.msie && ($.browser.version == "6.0"));
        var datas = null;
        if(isIE6)
            datas = JSON.parse(memory.getItem(action.substring(index + 1,lIndex > -1 ? lIndex : action.length + 1)));/*historyMap[action]; */
        else
		...
		if (datas == null) {
            ...
            if(isIE6) {
                memory.setItem(action.substring(index + 1,lIndex > -1 ? lIndex : action.length + 1),JSON.stringify($form.formToArray()));
            }else
		...
		for ( var i = 0; i < datas.length; i++) {
			...
			if (data.name != '_history' && !$form.find('[name="' + data.name + '"]').length) {
				fieldsArray.push('<input type="hidden" name="' + data.name + '" value="' + (data.value=='null' ? '' : data.value) + '" >')
			}
		}
