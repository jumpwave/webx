(function($) {
	...
	myflow.util = {
		...
		nextNum :(function(ctype) { // 记录每种类型，当前个数，用于编号
			var map = {};
			return function(ctype) {
				if(null == map[ctype]){ // 第一个
					map[ctype] = 1;
					return '';
				}else{
					return ++map[ctype];
	...
	myflow.rect = function(o, r, id) {
		var _this = this, 
		_o = $.extend(true, {}, myflow.config.rect, o), 
		_num = myflow.util.nextNum(_o.props['ctype']), 
...
//			}
	...
	myflow.path = function(o, r, from, to) {
				...
				switch (_t) {
					...
					case 'small' :// 移动小点时，转变为大点，增加俩个小点
						if (_lt && _rt && !myflow.util.isLine(_lt.pos(), {
							...
							var lt = new dot('small', myflow.util.center(_lt.pos(), _pos), _lt, _lt.right());
							_lt.right(lt);
							_lt = lt;

							var rt = new dot('small', myflow.util.center(_rt.pos(), _pos), _rt.left(), _rt);
							_rt.left(rt);
							_rt = rt;
		...
		// 转化json数据
		this.toJson = function() {
			var data = "{from:'" + _from.text() + "',to:'" + _to.text()
	...
	myflow.props = function(o, r) {
		var _this = this, _pdiv = $('#myflow_props').hide().css(myflow.config.props.attr).bind('click', function() {
			return false;
		...
		var showpropsHandler = function(e, props, src) {
			if (_src && _src.getId() == src.getId()) {// 连续点击不刷新
				//类似 双击事件
				myflow.config.tools.showProps.ondbclick(e, props, src, function(name, data){
					// 修改步骤在表格中的显示名称
					$(_r).trigger('textchange', [name, src]);
					// 缓存当前步骤的属性值
					for(var key in data){
						props[key] = data[key];
					}
...
//			_tb.empty();
			...
			//点击空白处，隐藏属性框
			//线，不展示属性框
			if(src.getId() == '00000000' || props['ctype'].value == 'path'){
				_pdiv.hide();
				return;
			}
			...
			// 自定义展示节点的属性
			myflow.config.tools.showProps.onclick(e, props, src, function(name, data){
...
//			}
	...
	myflow.editors = {
		textEditor : function() {
			...
			this.init = function(props, k, div, src, r) {
				...
				$('<input  style="width:100%;"/>').val(_src.text()).change(function() {
	...
	myflow.init = function(c, o) {
		...
		var _w = myflow.config.width, _h = myflow.config.height, _r = Raphael(c, _w * 1.0, _h * 1.0), _states = {}, _paths = {};
		...
		$(document).click(function() {
			...
			$(_r).trigger('click', {getId : function() {
					return '00000000';
				}
			});
 			// 注释掉，禁止任意点击就弹出属性框
		...
		$(_r).bind('addrect', function(e, type, o, id) {
			//拖动图标，在画板上画出大图表，这个获取的
			// $('body').append(type+', ');r
			var rect = new myflow.rect($.extend(true, {}, myflow.config.tools.states[type], o), _r, id)
		...
		if (myflow.config.editable) {
			// 工具栏
			$("#myflow_tools").css(myflow.config.tools.attr);
			...
			$(c).droppable({
				...
				drop : function(event, ui) {
					...
					// 由于画板不是全屏，这里的计算需要减去响应的x,y的距离，不然拖动出来的图标不是鼠标所在位置
					$(_r).trigger('addrect', [ui.helper.attr('type'), {
						attr : {
							x : ui.helper.offset().left - $(this).offset().left,
							y : ui.helper.offset().top - $(this).offset().top
						}
					},id]);
...
//					if (_states[k]) {
//						data += _states[k].text() + ':' + _states[k].toJson() + ',';
//					}
//				}
//				if (data.substring(data.length - 1, data.length) == ',')
//					data = data.substring(0, data.length - 1);
...
//				if (data.substring(data.length - 1, data.length) == ',')
//					data = data.substring(0, data.length - 1);
//				data += '},"props":{"directory": "/","name": "test3"';
//				data += '}}';
//				
...
//			});
		...
		if (o.restore) {
			...
			if (data.states) {
				for (var k in data.states) {
					var rect = new myflow.rect($.extend(true,{},myflow.config.tools.states[data.states[k].type],data.states[k]), _r);
			...
			if (data.paths) {
				for (var k in data.paths) {
					var p = new myflow.path($.extend(true, {},myflow.config.tools.path, data.paths[k]),_r, rmap[data.paths[k].from],rmap[data.paths[k].to]);
		...
		// 组装数据,用于获取 表输入的信息
		this.getMergeJoinData = function(id) {
			var states = [];
			for (var k in _paths) {
				if(null != _paths[k] && _paths[k].to().getId() == id ){
					// 指向本节点的节点的 表输入信息
					states.push(_paths[k].from());
				}
			}
			return states;
		};
		...
		// 获取当前图中的数据, 用于保存
		this.getGraphJsonData = function(name){
			var targetStates;
			var data = '{"states":{';
			for (var k in _states) {
				if (_states[k]) {
					data += _states[k].text() + ':' + _states[k].toJson() + ',';
					if(_states[k].getProps().ctype == 'InsertUpdate'){
						targetStates = _states[k].text();
					}
				}
			}
			if (data.substring(data.length - 1, data.length) == ',')
			...
			if (data.substring(data.length - 1, data.length) == ',')
				data = data.substring(0, data.length - 1);
			data += '},"props":{"directory": "/","targetStates":"' + targetStates + '","name":"' + name + '"';
			data += '}}';
	...
	$.fn.myflow = function(o) {
		this.each(function() {
			myflow.data = myflow.init(this, o);
		});
		return myflow;
	};
