	...
	initComponent: function() {
		var menu = new Ext.menu.Menu({
			items: [{
	        	...
	        	text: '重命名', scope: this, handler: this.rename
	        }, {
	...
	rename: function() {
		...
		if(node) {
			...
			if(!path||"/"==path){
		    	...
		    	Ext.Msg.prompt('系统提示', '请输入新名称:', function(btn, text){
				    if (btn == 'ok' && text != '') {
				    	Ext.getBody().mask('正在修改，请稍后...');
				    	Ext.Ajax.request({
							url: GetUrl('repository/rename.do'),
							method: 'POST',
							params: {type:node.attributes.type,path:path,oldName:node.attributes.text, newName: text},
							success: function(response) {
								try {
